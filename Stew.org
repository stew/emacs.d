...
(use-package git-link
  :ensure t
  :config (defun git-link-tgt (hostname dirname filename branch commit start end)
  (format "https://git.target.com/%s/blob/%s/%s"
	  dirname
	  (or branch commit)
	  (concat filename
                  (when start
...
(add-to-list 'load-path "~/src/lsp-scala")
(require 'lsp-scala)
